(function (){
  ...
  var Nemesis = Asteroids.Nemesis = function (game){
    ...
    var img_nemesis = new Image();
    img_nemesis.src = './img/nemesis.png';
    this.img_nemesis = img_nemesis;
  ...
  Util.inherits(Nemesis, Asteroids.MovingObject);
  ...
  Nemesis.prototype.relocate = function() {
    this.safe = true;
    this.shields = 3;
  ...
  Nemesis.prototype.power = function(thrust) {
    // velocity
    var x = this.vel[0]
    var y = this.vel[1]
  ...
  Nemesis.prototype.fireBullet = function() {
  ...
  Nemesis.prototype.rotateLeft = function () {
    this.heading += Math.PI/20;

  };

  Nemesis.prototype.rotateRight = function () {
    this.heading -= Math.PI/20;
  };

  Nemesis.prototype.emergencyBrake = function (){
    this.vel = [0,0];
    this.shields = 0;
  };
  ...
  Nemesis.prototype.draw = function (ctx) {
    ...
    //     || this.safe === false){
      ...
      ctx.translate(this.pos[0], this.pos[1]);
      ctx.rotate(-this.heading);
      ctx.drawImage(this.img_nemesis, -20, -20)
      ctx.restore();
      // ship sprite success
    // }
