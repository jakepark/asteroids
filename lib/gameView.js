(function (){
  ...
  var GameView = Asteroids.GameView = function(game, ctx, player2) {
    if (player2){
      this.h2h = true;
    }
    this.game = game;
  ...
  GameView.prototype.start = function () {
    ...
    var gameLoop = setInterval(function (){
      ...
      if (view.game.gamedone === true){
        ...
        var divOver = document.getElementById("ending")

        if (view.game.ship.lives > 0){
          var winner = document.getElementById("winner-player")
        } else {
          var winner = document.getElementById("winner-nemesis")
        }
        winner.style.visibility = 'visible';

        var shotsFired = view.game.ship.spammed
        var shotsHit = view.game.ship.hits
        var kills = view.game.ship.kills
        var $div = $(document.getElementsByClassName('player_stats'))
        $div.append('Shots fired: ' + shotsFired + '<br>')
        $div.append('Shots hit ' + shotsHit + '<br>')
        $div.append('Kills: ' + kills + '<br>')
  ...
  GameView.prototype.updatePlayer1 = function () {
    ...
    shieldbar.max = Asteroids.Ship.SHIELDMAX;
